function [GlobalMin, fitness, GlobalParams,time] = PROPOSED(coyotes, FOBJ, lu, Uu, nfevalMAX)
%% ------------------------------------------------------------------------
...
%repmat(VarMin,pop_total,1) + rand(pop_total,D).*(repmat(VarMax,pop_total,1) - repmat(VarMin,pop_total,1));
...
while nfeval<=nfevalMAX % Stopping criteria
    ...
    for p=1:n_packs
        ...
        Ps  = 1/D*((mean(costs_aux)-costs_aux(p))/iworst);
