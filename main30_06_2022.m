...
if an == 1
    ...
    for i = 1 : 5
        ...
        featuresTrain{i} = activations(net_cnn{i}, Feat{i},'avgpool2d_2','OutputAs','rows'); %  Features extracted from Pooling Layer
    end
...
%% $$$$$$$$$$$  MULTI-OBJECTIVE CNN-BASED FEATURE EXTRACTION AND IMPROVED DNN-LSTM BASED LOCALIZATION $$$$$$$$$$$$$ %%
an = 0;                                                    % Set 1 to recompute
if an == 1
    load featuresTrain
    load Scores
    ...
    Npop = 10;
    Ch_len = 19;                                            % 16 for weight optimized  CNN , 2 for DNN hidden neuron and 1 for LSTM hidden neurons optimization
    xmin = [zeros(Npop, 16) 5.*ones(Npop, 2) 5.*ones(Npop, 1) ];
    xmax = [ones(Npop, 16) 30.*ones(Npop, 2) 255.*ones(Npop, 1) ];
    ...
    for i = 1 : 5                                          % for all Datasets
        ...
        Gwo_Sfo(i).bf = bestfit; Gwo_Sfo(i).fit = fitness; Gwo_Sfo(i).bs = bestsol; Gwo_Sfo(i).ct = time;
        save Gwo_Sfo Gwo_Sfo
        ...
        disp('COA')
        [bestfit,fitness,bestsol,time] = COA(initsol,fname,xmin,xmax,itermax);               % COA
        Coa(i).bf = bestfit; Coa(i).fit = fitness; Coa(i).bs = bestsol; Coa(i).ct = time;
        save Coa Coa
        
        disp('PROPOSED')
        [bestfit,fitness,bestsol,time] = PSO(initsol,fname,xmin,xmax,itermax);            % Improved COA
    ...
    for i =  1 : 5 % for all Datasets
        ...
        for rk = 1 : 4
            for j = 1 : 5 % for all algorithms
                ...
                sol = alg(i).bs;
                Eval{i,rk, j} = Model_CNN_DNN_LSTM_prop(Feat1, Tar, sol, per(rk), pos);   %% Model CNN + DNN + LSTM Optimized
            end
            load Feat
            Feat1 = Feat{i}(:, :, :,ind(1)*20 - 19:ind(end)*20);
            Eval{i,rk, 6} = Model_CNN(Feat1, Tar, per(rk), pos);                     %% Model CNN
            Feat1 = cnn_Feat(ind(1)*20 - 19:ind(end)*20, :);
            Eval{i,rk, 7} = Model_CNN_DNN(Feat1, Tar,  per(rk), pos);                %% Model CNN + DNN
            load Data
            Feat1 = Data{i}(ind(1)*20 - 19:ind(end)*20, :);
            Eval{i,rk, 8} = Model__LSTM(Feat1, Tar,  per(rk), pos);                  %% Model LSTM
            Feat1 = cnn_Feat(ind(1)*20 - 19:ind(end)*20, :);
            Eval{i,rk, 9} = Model_CNN_DNN_LSTM(Feat1, Tar,  per(rk), pos);           %% Model CNN + DNN + LSTM Without Optimization
        end
    end
    save Eval Eval
end
